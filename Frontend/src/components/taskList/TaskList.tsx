...
function TaskList({ title }: taskInterface) {
  ...
  return (
    <div className="bg-[#1b1b1b] w-[300px] px-3 py-3 rounded-2xl">
      <h1 className="text-slate-800 dark:text-gray-300 mb-2 px-5 font-medium">
        {title}
      </h1>
      <div className="my-2">
        {tasks.map((task) => (
          <div className="bg-slate-800 px-3 py-[6px] rounded cursor-pointer hover:brightness-125 ">
            <p className="text-slate-800 dark:text-gray-300 font-normal">{task.title}</p>
          </div>
        ))}
      </div>
